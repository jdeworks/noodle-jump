...
export function drawPowerUp(
  ...
  if (isNegative) {
    // Warning triangle shape with red glow
    // Outer glow
    gfx.moveTo(s, -4);
    gfx.lineTo(size + 4, size + 2);
    gfx.lineTo(-4, size + 2);
    gfx.closePath();
    gfx.fill({ color: 0xff0000, alpha: 0.15 });
    ...
    gfx.lineTo(0, size);
    gfx.closePath();
    gfx.stroke({ width: 2, color: 0x660000 });

    // Exclamation mark
    gfx.roundRect(s - 1, s - 2, 2, 6, 1);
    gfx.fill(0xffffff);
    gfx.circle(s, s + 6, 1.2);
    gfx.fill(0xffffff);
  } else {
    // Positive: diamond shape
    // Outer glow
    gfx.moveTo(s, -3);
    gfx.lineTo(size + 3, s);
    gfx.lineTo(s, size + 3);
...
function drawPowerUpIcon(gfx: Graphics, s: number, type: string): void {
  switch (type) {
    case "spaghetti_spring": {
      // Coiled spring — 4 loops
      for (let i = 0; i < 4; i++) {
        const y = s - 5 + i * 3;
        gfx.moveTo(s - 4, y);
        gfx.quadraticCurveTo(s, y - 2, s + 4, y);
        gfx.quadraticCurveTo(s, y + 2, s - 4, y + 1.5);
      }
      gfx.stroke({ width: 1.5, color: 0x8b6914 });
      // End caps
      gfx.circle(s - 4, s - 5, 1.5);
      gfx.circle(s + 1, s + 6, 1.5);
      gfx.fill(0x8b6914);
      break;
    }
    case "fusilli_tornado": {
      // Spiral tornado with motion lines
      for (let a = 0; a < Math.PI * 3; a += 0.4) {
        const r = 1 + a * 0.9;
        const px = s + Math.cos(a) * r;
        const py = s + Math.sin(a) * r * 0.6;
        gfx.circle(px, py, 1);
      }
      gfx.fill(0x333333);
      // Motion lines
      ...
      gfx.moveTo(s - 4, s + 3);
      gfx.lineTo(s - 7, s + 3);
      gfx.stroke({ width: 0.8, color: 0x333333, alpha: 0.5 });
      break;
    }
    case "ravioli_rocket": {
      // Rocket with nose cone, body, fins, and fire
      // Body
      gfx.roundRect(s - 2, s - 3, 4, 7, 1);
      gfx.fill(0x333333);
      // Nose cone
      gfx.moveTo(s, s - 6);
      gfx.lineTo(s + 2, s - 3);
    ...
    case "lasagna_layers": {
      // Stacked colored layers with wavy cheese
      const colors = [0xcc6600, 0xffcc44, 0xcc3333, 0xffcc44, 0xcc6600];
      for (let i = 0; i < 5; i++) {
        gfx.roundRect(s - 5, s - 4 + i * 2.2, 10, 2, 0.5);
        gfx.fill(colors[i]);
      }
      // Melting cheese drip
      gfx.moveTo(s + 3, s + 5);
      gfx.quadraticCurveTo(s + 4, s + 7, s + 3, s + 8);
      gfx.stroke({ width: 1.2, color: 0xffcc44 });
      break;
    }
    case "pepper_sneeze": {
      // Chili pepper shape
      gfx.moveTo(s, s - 5);
      gfx.quadraticCurveTo(s + 5, s - 2, s + 3, s + 3);
      gfx.quadraticCurveTo(s + 1, s + 6, s - 1, s + 4);
      gfx.quadraticCurveTo(s - 4, s + 1, s - 3, s - 3);
      gfx.quadraticCurveTo(s - 2, s - 5, s, s - 5);
      gfx.fill(0x8b0000);
      // Stem
      gfx.moveTo(s - 1, s - 5);
      gfx.lineTo(s, s - 7);
      ...
      // Sneeze particles
      gfx.circle(s + 5, s - 4, 0.8);
      gfx.circle(s + 6, s - 2, 0.8);
      gfx.circle(s + 4, s - 1, 0.8);
      gfx.fill(0xffaa00);
      break;
    }
    case "meatball_magnet": {
      ...
      gfx.moveTo(s - 4, s - 5);
      gfx.lineTo(s - 4, s + 1);
      gfx.quadraticCurveTo(s, s + 6, s + 4, s + 1);
      gfx.lineTo(s + 4, s - 5);
      gfx.lineTo(s + 2, s - 5);
      gfx.lineTo(s + 2, s + 1);
      gfx.quadraticCurveTo(s, s + 3.5, s - 2, s + 1);
      gfx.lineTo(s - 2, s - 5);
      gfx.closePath();
      gfx.fill(0xcc0000);
      // Blue tip
      gfx.rect(s + 2, s - 5, 2, 3);
      gfx.fill(0x0044cc);
      // Field lines
      gfx.moveTo(s - 1, s - 6);
      gfx.quadraticCurveTo(s, s - 8, s + 1, s - 6);
      gfx.stroke({ width: 0.7, color: 0x333333, alpha: 0.4 });
      break;
    }
  }
}
