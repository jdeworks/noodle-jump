...
import { ACHIEVEMENTS, loadAchievements } from "../systems/Achievements";
...
import { promptCode, doExport, doImport } from "./CustomizeActions";
import {
  addSection,
  addCosmeticRow,
  addAchievementRow,
...
export class CustomizeScreen {
  ...
  private render(): void {
    ...
    y = addInfoText(
      this.scrollContent,
      gw,
      "Progress is local — Export to back up before clearing browser data.",
      y,
      t,
    );
    ...
    for (let i = 0; i < trails.length; i++) {
      ...
      addGridItem(
        ...
        gw,
        c.name,
        this.cosmeticState.unlocked.has(c.id),
        this.cosmeticState.equipped.trail === c.id,
        i % 3,
        trailStartY + Math.floor(i / 3) * 32,
        equipFn(c.id),
        t,
      );
    }
    ...
    for (let i = 0; i < tints.length; i++) {
      ...
      addGridItem(
        ...
        c.name,
        this.cosmeticState.unlocked.has(c.id),
        this.cosmeticState.equipped.tint === c.id,
        i % 3,
        tintStartY + Math.floor(i / 3) * 32,
        equipFn(c.id),
        t,
        TINT_COLORS[c.id],
      );
    ...
    for (const c of getCosmeticsByType("theme")) {
      y = addCosmeticRow(
        ...
        c,
        this.cosmeticState.unlocked.has(c.id),
        this.cosmeticState.equipped.theme === c.id,
        y,
        equipFn(c.id),
        t,
      );
  ...
  private buildUI(oldContent: Container): void {
    ...
    bg.fill({ color: t.bg, alpha: t.bgAlpha });
    ...
    mask.rect(0, 40, GAME_WIDTH, GAME_HEIGHT - 40 - BOTTOM_H);
    mask.fill(0xffffff);
    ...
    bar.rect(0, GAME_HEIGHT - BOTTOM_H, GAME_WIDTH, BOTTOM_H);
    bar.fill({ color: t.bg, alpha: 0.95 });
    ...
    backBtn.y = GAME_HEIGHT - BOTTOM_H + 14;
  ...
  private bindScroll(overlay: Graphics): void {
    ...
    const endDrag = (e: { globalX: number; globalY: number }) => {
      ...
      if (this.totalDragDist < TAP_THRESHOLD) {
        ...
        for (const r of this.tapRegions) {
          if (
            e.globalY >= 40 &&
            e.globalY <= GAME_HEIGHT - BOTTOM_H &&
            sx >= r.x &&
            sx < r.x + r.w &&
            sy >= r.y &&
            sy < r.y + r.h
    ...
    overlay.on("wheel", (e) => {
      this.scrollY = Math.max(
        0,
        Math.min(this.maxScroll, this.scrollY + (e as unknown as WheelEvent).deltaY),
      );
      this.scrollContent.y = 40 - this.scrollY;
      this.scrollVelocity = 0;
    });
  ...
  private startMomentum(): void {
    ...
    requestAnimationFrame(
      ...
      }.bind(null, this.tickerFn),
    );
  }

  private promptCode(): void {
    promptCode(this.cosmeticState, (s) => {
      this.cosmeticState = s;
      this.render();
    });
  }
