...
export function showTitleScreen(
  ...
  const charName = new Text({
    text: CHARACTERS.find((c) => c.id === getSelectedCharacter())?.name ?? "Chef",
    style: new TextStyle({
      fontFamily: "monospace",
      fontSize: 11,
      fill: uiT.accent,
  ...
  charName.x = GAME_WIDTH / 2;
  charName.y = cursorY + 42;
  charName.anchor.set(0.5, 0);
  contentGroup.addChild(charName);

  const arrowSt = new TextStyle({
    fontFamily: "monospace",
    fontSize: 20,
    fill: "#999999",
    stroke: { color: "#000000", width: 2 },
  ...
  leftArr.x = GAME_WIDTH / 2 - 50;
  leftArr.y = cursorY + 28;
  leftArr.anchor.set(0.5, 0.5);
  leftArr.eventMode = "static";
  leftArr.cursor = "pointer";
  contentGroup.addChild(leftArr);
  const rightArr = new Text({ text: "\u25B6", style: arrowSt });
  rightArr.x = GAME_WIDTH / 2 + 50;
  rightArr.y = cursorY + 28;
  rightArr.anchor.set(0.5, 0.5);
  rightArr.eventMode = "static";
  rightArr.cursor = "pointer";
  contentGroup.addChild(rightArr);
  ...
  leftArr.on("pointertap", (e: Event) => {
    e.stopPropagation();
    cycleChar(-1);
  });
  rightArr.on("pointertap", (e: Event) => {
    e.stopPropagation();
    cycleChar(1);
  });
  cursorY += 60;
  ...
  function mkBtn(label: string, y: number, fc: number, tc: string, fs: number) {
    ...
    bg.fill({ color: fc, alpha: 0.7 });
    bg.roundRect(btnX, y, btnW, btnH, 10);
    bg.stroke({ width: 1.5, color: uiT.buttonBorder, alpha: 0.6 });
    bg.eventMode = "static";
    bg.cursor = "pointer";
    ...
    t.eventMode = "static";
    t.cursor = "pointer";
    contentGroup.addChild(t);
    return { bg, text: t };
  }

  const playBtn = mkBtn("Tap to Play", cursorY, uiT.buttonBg, uiT.text, 20);
  const promptText = playBtn.text;
  cursorY += btnH + btnSp;
  const dailyBtn = mkBtn("Daily Challenge", cursorY, uiT.buttonBg, uiT.accent, 15);
  cursorY += btnH + btnSp;
  const mpBtn = mkBtn("Multiplayer", cursorY, uiT.buttonBg, uiT.accent, 16);
  cursorY += btnH + btnSp;
  const howBtn = mkBtn("How to Play", cursorY, uiT.buttonBg, uiT.text, 15);
  cursorY += btnH + btnSp;
  const customBtn = mkBtn("Custom Run", cursorY, uiT.buttonBg, uiT.text, 15);
  cursorY += btnH + btnSp;
  const custBtn = mkBtn("Customize", cursorY, uiT.buttonBg, uiT.accent, 15);
  cursorY += btnH + btnSp;

  // Fullscreen icon (top-right corner)
  const isIOS = /iPad|iPhone|iPod/.test(navigator.userAgent);
  const isPWA = window.matchMedia("(display-mode: standalone)").matches;
  const fsIcon = new Text({
    text: isIOS && !isPWA ? "" : "\u26F6",
    style: new TextStyle({
      fontFamily: "monospace",
      fontSize: 20,
  ...
  if (!isIOS || isPWA) {
    fsIcon.x = GAME_WIDTH - 14;
    fsIcon.y = 10;
    fsIcon.anchor.set(1, 0);
    fsIcon.eventMode = "static";
    fsIcon.cursor = "pointer";
    fsIcon.on("pointertap", (e: Event) => {
      e.stopPropagation();
      requestFullscreen();
    });
  ...
  // Input hint
  const hint =
    "ontouchstart" in window ? "Tilt or tap left/right to move" : "Arrow keys / WASD to move";
  ...
  hintT.x = GAME_WIDTH / 2;
  hintT.y = cursorY;
  hintT.anchor.set(0.5, 0);
  contentGroup.addChild(hintT);
  cursorY += 38;

  // Settings
  const stCont = createSettingsToggles();
  stCont.y = cursorY - 5;
  contentGroup.addChild(stCont);
  ...
  let scrollY = 0,
    animTick = 0,
    titleDestroyed = false;
  const titleTicker = () => {
    if (titleDestroyed || !chefGfx.context) {
  ...
  const menus = setupTitleMenus(
    {
      ...
      parallax,
      onStartGame,
      getTitleTicker: () => titleTicker,
      getHandleKey: () => handleKey,
      setTitleDestroyed: () => {
        titleDestroyed = true;
      },
    },
    {
      howBg: howBtn.bg,
      howText: howBtn.text,
      customBg: customBtn.bg,
      customText: customBtn.text,
      custBg: custBtn.bg,
      custText: custBtn.text,
      dailyBg: dailyBtn.bg,
      dailyText: dailyBtn.text,
      mpBg: mpBtn.bg,
      mpText: mpBtn.text,
    },
  );
  ...
  const startGame = async () => {
    if (started) return;
    if (
      menus.explanationScreen.isActive() ||
      menus.customRunScreen.isActive() ||
      menus.customizeScreen.isActive() ||
      menus.dailyChallengeScreen.isActive()
    )
      return;
    started = true;
