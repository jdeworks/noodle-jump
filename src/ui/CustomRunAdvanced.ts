...
export function addAdvancedDebugRows(
  ...
  y = addRow(
    ...
    tapRegions,
    gameWidth,
    `Force PowerUp: ${currentForce.replace(/_/g, " ")}`,
    "[Next]",
    y,
    () => {
      const idx = allTypes.indexOf(currentForce);
      debugCfg.forcePowerUpType =
        allTypes[(idx + 1) % allTypes.length] === "(none)"
          ? null
          : allTypes[(idx + 1) % allTypes.length];
      onRender();
    },
  );
  ...
  y = addRow(
    ...
    tapRegions,
    gameWidth,
    `Force Platform: ${curPlat.replace(/_/g, " ")}`,
    "[Next]",
    y,
    () => {
  ...
  y = addRow(
    ...
    tapRegions,
    gameWidth,
    `Force Boss: ${curBoss.replace(/_/g, " ")}`,
    "[Next]",
    y,
    () => {
      const idx = bossTypes.indexOf(curBoss);
      debugCfg.forceBossType =
        bossTypes[(idx + 1) % bossTypes.length] === "(none)"
          ? null
          : bossTypes[(idx + 1) % bossTypes.length];
      onRender();
    },
  );
  ...
  y = addRow(
    ...
    tapRegions,
    gameWidth,
    `Boss At Plat: ${debugCfg.forceBossAtPlatforms === 0 ? "OFF" : debugCfg.forceBossAtPlatforms}`,
    "[Cycle]",
    y,
    () => {
      const opts = [0, 10, 25, 50, 100, 200];
      debugCfg.forceBossAtPlatforms =
        opts[(opts.indexOf(debugCfg.forceBossAtPlatforms) + 1) % opts.length];
      onRender();
    },
  );
  ...
  y = addRow(
    ...
    tapRegions,
    gameWidth,
    `Enemy Spawn: ${debugCfg.enemySpawnMultiplier.toFixed(1)}x`,
    "[Cycle]",
    y,
    () => {
  ...
  y = addRow(
    ...
    tapRegions,
    gameWidth,
    `Boss Attack: ${debugCfg.bossAttackMultiplier.toFixed(1)}x`,
    "[Cycle]",
    y,
    () => {
  ...
  y = addToggleRow(
    ...
    gameWidth,
    "Disable Weather",
    debugCfg.disableWeather,
    y,
    () => {
      debugCfg.disableWeather = !debugCfg.disableWeather;
      onRender();
    },
  );
  y = addToggleRow(
    ...
    gameWidth,
    "Disable Parallax",
    debugCfg.disableParallax,
    y,
    () => {
      debugCfg.disableParallax = !debugCfg.disableParallax;
      onRender();
    },
  );
  y = addToggleRow(
    ...
    gameWidth,
    "Disable Particles",
    debugCfg.disableEffectParticles,
    y,
    () => {
      debugCfg.disableEffectParticles = !debugCfg.disableEffectParticles;
      onRender();
    },
  );
  return y;
