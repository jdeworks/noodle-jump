...
function targetVol(): number {
  return musicVol() * BASE_MUSIC_VOLUME;
}
  ...
  const fadeOut = trackInterval(
    setInterval(() => {
      ...
      if (musicElement.volume <= 0.01) {
        ...
        musicElement.play().catch(() => {});
        const fadeIn = trackInterval(
          setInterval(() => {
            if (!musicElement || !musicPlaying) {
              removeInterval(fadeIn);
              return;
            }
            const tv = targetVol();
...
export function playBossMusic(): void {
  ...
  if (!bossMusicElement) {
    bossMusicElement = new Audio(BOSS_TRACK);
    bossMusicElement.loop = true;
  }
  bossMusicElement.volume = 0;
  bossMusicElement.play().catch(() => {});
  bossMusicPlaying = true;
  const fi = trackInterval(
    setInterval(() => {
      if (!bossMusicElement) {
        removeInterval(fi);
        return;
