...
export type PowerUpType = 'spaghetti_spring' | 'fusilli_tornado' | 'ravioli_rocket' | 'lasagna_layers' | 'pepper_sneeze'

const POWERUP_TYPES: PowerUpType[] = ['spaghetti_spring', 'fusilli_tornado', 'ravioli_rocket', 'lasagna_layers', 'pepper_sneeze']
...
export function createPowerUp(platform: PlatformState, type?: PowerUpType): PowerUpState {
  return {
...
export function spawnPowerUps(platforms: PlatformState[]): PowerUpState[] {
  const powerUps: PowerUpState[] = []
  let cooldown = 0

  for (let i = 0; i < platforms.length; i++) {
    const platform = platforms[i]
    // Skip early platforms — let the player get going first
    ...
    if (Math.random() < POWERUP_SPAWN_CHANCE) {
      powerUps.push(createPowerUp(platform))
...
export function collectPowerUps(
  ...
  for (let i = 0; i < powerUps.length; i++) {
    ...
    const overlapX = player.x < pu.x + pu.size && player.x + player.width > pu.x
    const overlapY = player.y < pu.y + pu.size && player.y + player.height > pu.y
  ...
  switch (type) {
    ...
    case 'fusilli_tornado':
      return {
        ...
        effect: { type: 'fusilli_tornado', ticksRemaining: FUSILLI_TORNADO_DURATION },
      }

    case 'ravioli_rocket':
      return {
        player: { ...player, vy: RAVIOLI_ROCKET_VELOCITY, isJumping: true },
        effect: { type: 'ravioli_rocket', ticksRemaining: RAVIOLI_ROCKET_DURATION },
      }

    case 'lasagna_layers':
      // Slow fall — gravity reduced to 20%, player floats gently
      return {
        player: { ...player, vy: SPAGHETTI_SPRING_VELOCITY * 0.5, isJumping: true },
        effect: { type: 'lasagna_layers', ticksRemaining: LASAGNA_LAYERS_DURATION },
      }
    ...
    case 'pepper_sneeze':
      ...
      return {
        ...
        effect: { type: 'pepper_sneeze', ticksRemaining: PEPPER_SNEEZE_DURATION },
      }
  }
}
...
export function tickEffect(
  ...
  if (effect.type === 'lasagna_layers') {
    // Slow fall — reduce gravity effect by capping downward velocity
    let vy = player.vy
    if (vy > 0) {
      vy *= LASAGNA_GRAVITY_MULTIPLIER // dramatically slow the fall
    }
    return {
      player: { ...player, vy },
      effect: updatedEffect,
